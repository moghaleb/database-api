...
// ======== قاعدة البيانات (في الذاكرة، مناسبة للاختبار) ========
...
// ======== جميع الـ APIs الموجودة في الكود الأصلي ========
// (تم تضمينها كاملةً، لكن اختصاراً للعرض نذكر أهمها)

app.get('/', (req, res) => {
  res.json({ status: 'success', message: '🚀 نظام الاختبار يعمل بنجاح!', endpoints: ['/admin', '/api/test', '/api/db-test', '/api/save-data', '/api/all-data', '/api/process-payment', '/api/orders', '/api/categories', '/api/perfumes'] });
});

app.get('/api/test', (req, res) => res.json({ status: 'success', message: '✅ تم الاتصال بالخادم بنجاح!' }));
...
app.post('/api/process-payment', (req, res) => {
  const { cart_items, total_amount, customer_name, customer_phone, payment_method, ...rest } = req.body;
  if (!cart_items || cart_items.length === 0) return res.status(400).json({ status: 'error', message: 'السلة فارغة' });
  const order_number = 'ORD-' + Date.now() + '-' + Math.floor(Math.random() * 1000);
  const order_date = new Date().toISOString();
  const sql = `INSERT INTO orders (order_number, cart_items, total_amount, discount_amount, coupon_code, gift_card_amount, order_date, order_status, customer_name, customer_phone, customer_email, customer_secondary_phone, payment_method, transfer_name, transfer_number, customer_address, address_city, address_area, address_detail, shipping_city, shipping_area, shipping_fee, final_amount, order_notes, expected_delivery, items_count, shipping_type) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
  const finalAmount = parseFloat(total_amount);
  db.run(sql, [order_number, JSON.stringify(cart_items), finalAmount, 0, null, 0, order_date, 'pending', customer_name, customer_phone, rest.customer_email || '', rest.customer_secondary_phone || '', payment_method || 'online', rest.transfer_name || '', rest.transfer_number || '', rest.customer_address || '', rest.address_city || '', rest.address_area || '', rest.address_detail || '', rest.shipping_city || '', rest.shipping_area || '', 0, finalAmount, rest.order_notes || '', rest.expected_delivery || '', cart_items.length, rest.shipping_type || 'توصيل منزلي'], function(err) {
    if (err) return res.status(500).json({ status: 'error', message: err.message });
...
app.get('/api/orders', (req, res) => {
  db.all('SELECT * FROM orders ORDER BY created_at DESC', (err, rows) => {
    if (err) return res.status(500).json({ status: 'error', message: err.message });
    res.json({ status: 'success', orders: rows.map(o => ({ ...o, cart_items: JSON.parse(o.cart_items) })) });
  });
});
app.put('/api/orders/:id/status', (req, res) => {
  ...
  db.run('UPDATE orders SET order_status = ? WHERE id = ?', [status, id], function(err) {
    if (err) return res.status(500).json({ status: 'error', message: err.message });
    res.json({ status: 'success', message: 'تم تحديث الحالة', updated_id: id });
  });
});

// كوبونات و قسائم
app.get('/api/validate-coupon', (req, res) => { /* كود التحقق من الكوبون */ res.json({ status: 'success', valid: true }); });
...
app.post('/api/coupons', (req, res) => {
  const { code, discount_value, discount_type, min_order_amount, valid_until, is_active } = req.body;
  if (!code) return res.status(400).json({ status: 'error', message: 'الكود مطلوب' });
  const valid_from = new Date().toISOString();
  db.run(`INSERT INTO coupons (code, discount_type, discount_value, min_order_amount, valid_from, valid_until, is_active) VALUES (?, ?, ?, ?, ?, ?, ?)`, [code, discount_type, discount_value, min_order_amount || 0, valid_from, valid_until || new Date(Date.now()+30*86400000).toISOString(), is_active !== undefined ? is_active : 1], function(err) {
    if (err) return res.status(500).json({ status: 'error', message: err.message });
    res.json({ status: 'success', message: 'تمت الإضافة', coupon_id: this.lastID });
  });
});
...
app.put('/api/coupons/:id', (req, res) => {
  const { is_active, used_count, discount_value, min_order_amount, valid_until } = req.body;
  const fields = [];
  const values = [];
...
app.post('/api/gift-cards', (req, res) => {
  const { card_number, pin_code, initial_amount, valid_until, customer_name, customer_phone, notes, is_active } = req.body;
  if (!card_number || !pin_code || !initial_amount) return res.status(400).json({ status: 'error', message: 'بيانات ناقصة' });
  db.run(`INSERT INTO gift_cards (card_number, pin_code, initial_amount, current_balance, valid_until, customer_name, customer_phone, notes, is_active) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, [card_number, pin_code, initial_amount, initial_amount, valid_until || new Date(Date.now()+90*86400000).toISOString(), customer_name || '', customer_phone || '', notes || '', is_active !== undefined ? is_active : 1], function(err) {
    if (err) return res.status(500).json({ status: 'error', message: err.message });
    res.json({ status: 'success', message: 'تمت الإضافة', gift_card_id: this.lastID });
  });
});
...
app.post('/api/validate-gift-card', (req, res) => {
  ...
  db.get('SELECT * FROM gift_cards WHERE card_number = ? AND pin_code = ? AND is_active = 1', [card_number, pin_code], (err, card) => {
    ...
    const now = new Date();
    if (now > new Date(card.valid_until)) return res.status(400).json({ status: 'error', message: 'انتهت الصلاحية' });
    const used = Math.min(card.current_balance, parseFloat(order_amount || 0));
    res.json({ status: 'success', valid: true, used_amount: used, remaining: card.current_balance - used });
  });
});
...
app.delete('/api/clear-all-data', (req, res) => {
  ...
  res.json({ status: 'success', message: 'تم مسح البيانات' });
});

// ======== قالب لوحة التحكم الاحترافية ========
...
app.get('/admin', (req, res) => {
  db.all('SELECT * FROM test_users ORDER BY created_at DESC', (err, rows) => {
    ...
    <div class="card"><div class="card-header"><h2><i class="fas fa-list"></i> قائمة المستخدمين</h2><button onclick="exportUsers()" class="btn btn-primary"><i class="fas fa-file-excel"></i> تصدير JSON</button></div><div class="card-body"><div class="table-responsive"><table><thead><tr><th>#</th><th>الاسم</th><th>البريد</th><th>الهاتف</th><th>الرسالة</th><th>التاريخ</th><th>الإجراءات</th></tr></thead><tbody>
      ${users.length===0?'<tr><td colspan="7" class="empty-state"><i class="fas fa-inbox"></i><br>لا توجد بيانات</td></tr>':
        users.map(u=>`<tr><td>${u.id}</td><td><strong>${u.name||'-'}</strong></td><td>${u.email||'-'}</td><td>${u.phone||'-'}</td><td>${u.message||'-'}</td><td>${new Date(u.created_at).toLocaleString('ar-SA')}</td><td><a href="/admin/purchases/${u.phone}?name=${encodeURIComponent(u.name)}" class="btn btn-primary" style="padding:6px 12px;font-size:12px"><i class="fas fa-shopping-bag"></i> مشتريات</a></td></tr>`).join('')}
...
app.get('/admin/orders', (req, res) => {
  db.all('SELECT * FROM orders ORDER BY created_at DESC', (err, orders) => {
    ...
    <script>async function updateOrderStatus(id,status){ const res=await fetch('/api/orders/'+id+'/status',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({status})}); const data=await res.json(); if(data.status==='success'){ showToast('تم تحديث الحالة'); setTimeout(()=>location.reload(),800); } else showToast(data.message,'error'); } function exportOrders(){ window.open('/api/export-all-sales','_blank'); }</script>`;
...
app.get('/admin/coupons', (req, res) => {
  db.all('SELECT * FROM coupons ORDER BY created_at DESC', (err, coupons) => {
    if (err) return res.status(500).send('خطأ');
    const couponsHtml = coupons.length===0 ? '<div class="empty-state"><i class="fas fa-ticket-alt"></i><br>لا توجد كوبونات</div>' : coupons.map(c=>`<div class="card"><div class="card-header"><div><i class="fas fa-tag"></i> <strong>${c.code}</strong> <span class="badge ${c.is_active && new Date(c.valid_until)>new Date() ? 'badge-success' : 'badge-danger'}">${c.is_active && new Date(c.valid_until)>new Date() ? 'نشط' : 'غير نشط'}</span></div><div>الاستخدام: ${c.used_count}/${c.max_uses===-1?'∞':c.max_uses}</div></div><div class="card-body"><div>الخصم: ${c.discount_value} ${c.discount_type==='percentage'?'%':'ر.س'} | الحد الأدنى: ${c.min_order_amount} ر.س</div><div>صالح حتى: ${new Date(c.valid_until).toLocaleDateString('ar-SA')}</div><div style="margin-top:15px;"><button onclick="deleteCoupon(${c.id})" class="btn btn-danger"><i class="fas fa-trash"></i> حذف</button> <button onclick="toggleCoupon(${c.id}, ${c.is_active?0:1})" class="btn btn-warning"><i class="fas fa-power-off"></i> ${c.is_active?'إيقاف':'تفعيل'}</button></div></div></div>`).join('');
    ...
    <div id="addCouponModal" class="modal"><div class="modal-content"><div class="modal-header"><h3>إضافة كوبون</h3><span class="close-modal" onclick="closeModal()">&times;</span></div><form id="couponForm"><div class="form-group"><label class="form-label">الكود</label><input name="code" class="form-control" required></div><div class="form-group"><label class="form-label">قيمة الخصم</label><input name="discount_value" type="number" class="form-control" required step="0.01"></div><div class="form-group"><label class="form-label">نوع الخصم</label><select name="discount_type" class="form-control"><option value="percentage">نسبة مئوية</option><option value="fixed">قيمة ثابتة</option></select></div><div class="form-group"><label class="form-label">الحد الأدنى للطلب</label><input name="min_order_amount" type="number" class="form-control" value="0"></div><div class="form-group"><label class="form-label">تاريخ الانتهاء</label><input name="valid_until" type="datetime-local" class="form-control" required></div><button type="submit" class="btn btn-primary">حفظ</button></form></div></div>
    <script>function showAddCouponModal(){ document.getElementById('addCouponModal').style.display='flex'; } function closeModal(){ document.getElementById('addCouponModal').style.display='none'; } document.getElementById('couponForm').addEventListener('submit', async(e)=>{ e.preventDefault(); const data=Object.fromEntries(new FormData(e.target)); data.discount_value=parseFloat(data.discount_value); data.min_order_amount=parseFloat(data.min_order_amount); data.valid_from=new Date().toISOString().slice(0,16); data.is_active=1; const res=await fetch('/api/coupons',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(data)}); const result=await res.json(); if(result.status==='success'){ showToast('تمت الإضافة'); location.reload(); } else showToast(result.message,'error'); }); async function deleteCoupon(id){ if(confirm('حذف الكوبون؟')){ await fetch('/api/coupons/'+id,{method:'DELETE'}); location.reload(); } } async function toggleCoupon(id,newStatus){ await fetch('/api/coupons/'+id,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({is_active:newStatus})}); location.reload(); }</script>`;
...
app.get('/admin/gift-cards', (req, res) => {
  db.all('SELECT * FROM gift_cards ORDER BY created_at DESC', (err, cards) => {
    ...
    <div id="addGiftModal" class="modal"><div class="modal-content"><div class="modal-header"><h3>إضافة قسيمة</h3><span class="close-modal" onclick="closeGiftModal()">&times;</span></div><form id="giftForm"><div class="form-group"><label>رقم القسيمة</label><input name="card_number" class="form-control" required></div><div class="form-group"><label>الرمز السري</label><input name="pin_code" class="form-control" required></div><div class="form-group"><label>المبلغ</label><input name="initial_amount" type="number" class="form-control" required step="0.01"></div><div class="form-group"><label>تاريخ الانتهاء</label><input name="valid_until" type="datetime-local" class="form-control" required></div><button type="submit" class="btn btn-primary">حفظ</button></form></div></div>
    <script>function showAddGiftCardModal(){ document.getElementById('addGiftModal').style.display='flex'; } function closeGiftModal(){ document.getElementById('addGiftModal').style.display='none'; } document.getElementById('giftForm').addEventListener('submit', async(e)=>{ e.preventDefault(); const data=Object.fromEntries(new FormData(e.target)); data.initial_amount=parseFloat(data.initial_amount); data.is_active=1; const res=await fetch('/api/gift-cards',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(data)}); const result=await res.json(); if(result.status==='success'){ showToast('تمت الإضافة'); location.reload(); } else showToast(result.message,'error'); }); async function deleteGiftCard(id){ if(confirm('حذف القسيمة؟')){ await fetch('/api/gift-cards/'+id,{method:'DELETE'}); location.reload(); } }</script>`;
